...
/**
 * Command to select and install a specific version of OLAF
 */
export async function selectVersionCommand(): Promise<void> {
    ...
    try {
        ...
        if (installNow === 'Install Now') {
            ...
            await vscode.commands.executeCommand('olaf.enhancedInstall');
        }
        ...
        if (errorMessage.includes('401') || errorMessage.includes('authentication')) {
            ...
            if (validate === 'Validate Access') {
                await vscode.commands.executeCommand('olaf.validateAccess');
