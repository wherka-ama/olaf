...
/**
 * Service for managing OLAF updates
 */
export class UpdateManager {
    ...
    /**
     * Update OLAF in a specific scope
     */
    ...
    public getUpdateNotificationMessage(updateResults: UpdateCheckResult[]): string {
        ...
        if (updatesAvailable.length === 0) {
            return 'OLAF is up to date';
        }

        if (updatesAvailable.length === 1) {
            const update = updatesAvailable[0];
            return `OLAF update available: ${update.currentVersion} → ${update.latestVersion} (${update.scope})`;
        }

        return `OLAF updates available for ${updatesAvailable.length} scopes`;
    }
